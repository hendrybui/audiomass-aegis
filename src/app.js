(function ( w, d ) {
	...
	function PKAE () {
		...
		q.init = function ( el_id ) {
			...
			// Initialize stem separation event listener
			if (q.stems) {
				q.listenFor('RequestStemSettings', function () {
					if (q.stems && q.stems.showSettingsModal) {
						q.stems.showSettingsModal();
					}
				});
			}
		...
		q._loadProject = function ( projectId ) {
			fetch (q._getApiBase () + '/projects/' + projectId).then (function (r) {
				...
				var clipBuffers = {};
					...
					// Load all clips in parallel for MUCH faster loading
					var total = clipIds.length;
					var clipBuffers = {};
					var loaded = 0;
					...
					toast.textContent = "Loading clips... 0/" + total + " (parallel - much faster!)";

					// Load all clips simultaneously instead of one by one
					...
					Promise.all (loadPromises).then (function () {
						...
						toast.textContent = "Error loading project!";
						setTimeout (function () { toast.remove (); }, 3000);
